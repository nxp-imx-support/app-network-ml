...
function updateDashboard(data) {
    ...
    document.getElementById('blacklist-updates').textContent = data.blacklist_updates || 0;
    
    // Update blacklist IPs
    updateBlacklistIps(data.blacklist_ips || []);
...
// Update blacklist IP list
function updateBlacklistIps(ips) {
    const container = document.getElementById('blacklist-ips-list');
    container.innerHTML = '';

    if (ips.length === 0) {
        container.innerHTML = '<div class="blacklist-ip-item">No blocked IPs</div>';
        return;
    }

    ips.forEach(ip => {
        const item = document.createElement('div');
        item.className = 'blacklist-ip-item';
        item.textContent = ip;
...
function updateCharts(data) {
    ...
    networkRxData.push((data.net_rx_bytes / (1024 * 1024)).toFixed(1));
    networkTxData.push((data.net_tx_bytes / (1024 * 1024)).toFixed(1));
