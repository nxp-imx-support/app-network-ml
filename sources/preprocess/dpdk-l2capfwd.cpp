...
/* Extract packets info and forward */
static void
l2fwd_cap_forward(struct rte_mbuf *pkt, unsigned portid) {
	unsigned dst_port;
	int sent;
...
l2fwd_main_loop(void)
{
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
				m = pkts_burst[j];
				// Maybe separate prefetch and forward, like l3fwd `l3fwd_em_no_opt_send_packets()`
				rte_prefetch0(rte_pktmbuf_mtod(m, void *));
				l2fwd_cap_forward(m, portid);
			}
		}
